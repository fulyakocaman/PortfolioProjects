...
bool place_in_hash_tables(string s) {
  ...
  while ((!placed) && (counter < 2 * tablesize)) {
    if (t[pos][index] == "") {
      ...
      if (index == 0) {
        index = 1; // The evicted value needs to move to the other table
      } else {
        index = 0; // The evicted value needs to move to the other table
      }
...
size_t f(string s, size_t index) {
  ...
  if (index == 0) {
    ...
    return val;
  } else { // executed when index == 1

    val = s[len - 1]; // start at the last char after the end of line character
    val = val % tablesize;
